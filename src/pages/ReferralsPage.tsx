...
const ReferralsPage: React.FC = () => {
  ...
  const referralKey =
    import.meta.env.VITE_REFERAL_KEY || "http://localhost:5173";
  ...
  useEffect(() => {
    const fetchReferralFriends = async () => {
      try {
        ...
        const source = Array.isArray(data) ? data : data.friends || [];
  ...
  const shareOnTelegram = () => {
    const message = encodeURIComponent(
      `Join MLM PLATFORM using my referral link: ${referralLink}`
    );
    window.open(
                ...
                referralFriends.map((friend) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-sm text-gray-700 dark:text-gray-300">
                      {new Date(friend.joinDate).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })}
